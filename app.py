...
def generate_cash_flow_statement(start_date, end_date):
    ...
    try:
        ...
        query = supabase.table('journal_entries').select('*').eq('account_code', '1-1101')
        if start_date:
        ...
        # Kas awal
        accounts = get_all_accounts()
        beginning_cash_account = next((a for a in accounts if a['account_code'] == '1-1101'), None)
...
def create_purchase(item_type, item_name, quantity, unit_price, total_amount, employee_username, receipt_image=''):
    ...
    try:
        ...
        if response.data:
            ...
            account_mapping = {
                'bibit': {
                    'debit': ('1-1301', 'Persediaan Ikan Mujair'),
                    'credit': ('1-1101', 'Kas')
                },
                'perlengkapan': {
                    'debit': ('1-1401', 'Perlengkapan'),
                    'credit': ('1-1101', 'Kas')
                },
                'peralatan': {
                    'debit': ('1-2101', 'Peralatan'),
                    'credit': ('1-1101', 'Kas')
                }
            }
...
def karyawan_edit_purchase(purchase_id):
    ...
    if request.method == 'POST':
        try:
            ...
            # Buat jurnal baru
            account_mapping = {
                'peralatan': ('1-2101', 'Peralatan'),
                'perlengkapan': ('1-1401', 'Perlengkapan'),
                'bibit': ('5-1201', 'Pembelian Bibit')
            }
            
            account_code, account_name = account_mapping.get(item_type, ('1-1401', 'Perlengkapan'))
    ...
    if request.method == 'POST':
        try:
            ...
            if not re.match(r'^[0-9]-[0-9]{4}$', account_code):
                flash('Format kode akun salah! Gunakan format X-XXXX (contoh: 1-1101)', 'error')
                return redirect(url_for('akuntan_accounts'))
                    ...
                    <form method="POST">
                        <div class="form-row">
                            <div class="form-group">
                                ...
                                <input type="text" name="account_code" required placeholder="1-1101" 
                                       pattern="[0-9]-[0-9]{{4}}"
...
def akuntan_manual_transaction():
    """Akuntan input transaksi manual - PERPETUAL METHOD"""
    if 'username' not in session or session.get('role') != 'akuntan':
        return redirect(url_for('login'))
    ...
    if request.method == 'POST':
        try:
            ...
            # ✅ MAPPING TRANSAKSI METODE PERPETUAL
            transaction_mapping = {
                'pembelian_bibit': {
                    'entries': [
                        {'account': ('1-1301', 'Persediaan Ikan Mujair'), 'debit': amount, 'credit': 0},
                        {'account': ('1-1101', 'Kas'), 'debit': 0, 'credit': amount}
                    ],
                    'desc': 'Pembelian bibit ikan mujair',
                    'icon': '🐟'
                },
                'pembelian_perlengkapan': {
                    'entries': [
                        {'account': ('1-1401', 'Perlengkapan'), 'debit': amount, 'credit': 0},
                        {'account': ('1-1101', 'Kas'), 'debit': 0, 'credit': amount}
                    ],
                    'desc': 'Pembelian perlengkapan budidaya',
                    'icon': '📦'
                },
                'pembelian_peralatan': {
                    'entries': [
                        {'account': ('1-2101', 'Peralatan'), 'debit': amount, 'credit': 0},
                        {'account': ('1-1101', 'Kas'), 'debit': 0, 'credit': amount}
                    ],
                    'desc': 'Pembelian peralatan budidaya',
                    'icon': '🔧'
                },
                'pembayaran_gaji': {
                    'entries': [
                        {'account': ('6-1101', 'Beban Gaji'), 'debit': amount, 'credit': 0},
                        {'account': ('1-1101', 'Kas'), 'debit': 0, 'credit': amount}
                    ],
                ...
                'pembayaran_listrik': {
                    'entries': [
                        {'account': ('6-1201', 'Beban Listrik'), 'debit': amount, 'credit': 0},
                        {'account': ('1-1101', 'Kas'), 'debit': 0, 'credit': amount}
                    ],
                ...
                'penerimaan_kas': {
                    'entries': [
                        {'account': ('1-1101', 'Kas'), 'debit': amount, 'credit': 0},
                        {'account': ('4-1201', 'Pendapatan Lain-lain'), 'debit': 0, 'credit': amount}
                    ],
                    'desc': 'Penerimaan kas lain-lain',
                    'icon': '💰'
                }
            }
            ...
            # ✅ SIMPAN SEMUA ENTRIES KE JURNAL UMUM (2 AKUN)
            for entry in mapping['entries']:
                create_journal_entry(
                    date=date,
                    account_code=entry['account'][0],
                    account_name=entry['account'][1],
                    description=description or mapping['desc'],
                    debit=entry['debit'],
                    credit=entry['credit'],
                    journal_type='GJ',
                    ref_code=ref_code
            ...
            # ✅ OTOMATIS KE INVENTORY JIKA PEMBELIAN BIBIT
            if transaction_type == 'pembelian_bibit':
                # Ambil saldo terakhir - PAKAI STRUKTUR LAMA
                last_entry = supabase.table('inventory_card')\
                    .select('*')\
                ...
                # Estimasi quantity jika tidak ada info (misal: total amount / 30000)
                estimated_qty = amount / 30000  # Asumsi harga per kg Rp30.000
                new_balance = last_qty + estimated_qty
                unit_price = 30000  # Default unit price
                
                # ✅ INSERT KE INVENTORY CARD - PAKAI STRUKTUR LAMA
                supabase.table('inventory_card').insert({
                    'date': date,
                    'product_name': 'Ikan Mujair',
                    'ref_code': ref_code,
                    'description': description or mapping['desc'],
                    'quantity_in': estimated_qty,
                    'quantity_out': 0,
                    'balance_quantity': new_balance,
                    'unit_price': unit_price,
                    'total_hpp': 0,
    ...
    # Ambil transaksi manual (yang ref_code-nya dimulai dengan 'MT')
    all_journals = get_journal_entries(journal_type='GJ')
    manual_transactions = []
    
    # Group by ref_code untuk tampilkan transaksi berpasangan
    grouped = {}
    for j in all_journals:
        ref = j.get('ref_code', '')
        if ref.startswith('MT'):
            if ref not in grouped:
                grouped[ref] = {'date': j['date'], 'description': j['description'], 'debit': [], 'credit': [], 'amount': 0}
            if j.get('debit', 0) > 0:
                grouped[ref]['debit'].append(f"{j['account_code']} - {j['account_name']}")
                grouped[ref]['amount'] = j['debit']
            if j.get('credit', 0) > 0:
                grouped[ref]['credit'].append(f"{j['account_code']} - {j['account_name']}")
    ...
    # Generate tabel transaksi
    transactions_html = ""
    for trans in manual_transactions[:20]:
        debit_str = "<br>".join(trans['debit'])
        credit_str = "<br>".join(trans['credit'])
        transactions_html += f"""
        <tr>
            <td>{trans['date']}</td>
            <td class="text-center"><code>{trans['ref_code']}</code></td>
            <td>{trans['description']}</td>
            <td style="font-size: 12px;">{debit_str}</td>
            <td style="font-size: 12px;">{credit_str}</td>
            <td class="text-right"><strong>{format_rupiah(trans['amount'])}</strong></td>
        </tr>
        """
    ...
    <body>
        <div class="dashboard-container">
            ...
            <div class="main-content">
                ...
                <div class="content-section" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; border-radius: 15px;">
                    ...
                    <p style="opacity: 0.9; margin-bottom: 20px;">Catat transaksi yang belum dilakukan oleh kasir atau karyawan</p>
                </div>
                ...
                <div class="content-section">
                    ...
                    <form method="POST">
                        ...
                        <div id="jurnalPreview" style="background: #f8f9fa; padding: 20px; border-radius: 10px; margin-top: 20px; display: none;">
                            <h3 style="color: #667eea; margin-bottom: 15px;">📝 Preview Jurnal</h3>
                            <table style="width: 100%; background: white;">
                                <thead>
                                    <tr style="background: #667eea; color: white;">
                                        <th style="padding: 10px;">Akun</th>
                                        <th class="text-right" style="padding: 10px;">Debit</th>
                                        <th class="text-right" style="padding: 10px;">Kredit</th>
                                    </tr>
                                </thead>
                        ...
                        <button type="submit" class="btn-sm btn-success btn-block" style="margin-top: 20px;">💾 Simpan Transaksi</button>
                    </form>
                </div>
                
                <div class="content-section">
                    <h2>📋 Riwayat Transaksi Manual</h2>
                    <table>
                        <thead>
                            <tr>
                                <th>Tanggal</th>
                        ...
                        <tbody>
                            {transactions_html if transactions_html else '<tr><td colspan="6" class="text-center">Belum ada transaksi manual</td></tr>'}
                        </tbody>
                    </table>
                </div>
            </div>
        ...
        function updatePreview() {{
            ...
            entries.forEach(entry => {{
                body.innerHTML += `
                    <tr>
                        <td style="padding: 10px;">${{entry.account}}</td>
                        <td class="text-right" style="padding: 10px;">${{entry.debit > 0 ? formatRupiah(entry.debit) : "-"}}</td>
                        <td class="text-right" style="padding: 10px;">${{entry.credit > 0 ? formatRupiah(entry.credit) : "-"}}</td>
                    </tr>
                `;
            }});
...
def akuntan_inventory_card():
    ...
    for card in card:
        ...
        purchase_amount = card.get('quantity_in', 0) * card.get('unit_price', 0)
        sales_amount = card.get('quantity_out', 0) * card.get('unit_price', 0)
        balance_amount = card.get('balance_quantity', 0) * card.get('unit_price', 0)
...
def akuntan_financial_statements():
    ...
    # Modal Awal
    modal_awal = worksheet_data.get('3-1101', {}).get('balance', 0)
    
    # Prive
    prive = worksheet_data.get('3-1201', {}).get('balance', 0)
...
def init_default_accounts():
    ...
    if len(accounts) == 0:
        default_accounts = [
            # ASET (1-xxxx)
            ('1-1101', 'Kas', 'aset', 'debit', 0),
            ('1-1201', 'Piutang Usaha', 'aset', 'debit', 0),
            ('1-1301', 'Persediaan Ikan Mujair', 'aset', 'debit', 0),
            ('1-1401', 'Perlengkapan', 'aset', 'debit', 0),
            ('1-2101', 'Peralatan', 'aset', 'debit', 0),
